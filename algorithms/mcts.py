...
class MonteCarloVRPPlanner:
    ...
    def _random_feasible_plan(
        ...
        for req in shuffled:
            placed = False
            vids = list(capacities_left.keys())
            self.rng.shuffle(vids)
            for vid in vids:
                if capacities_left[vid] >= req.demand:
                    routes[vid].stops.append(req)
                    capacities_left[vid] -= req.demand
                    placed = True
                    break
            if not placed:
                raise RuntimeError("Cannot assign all requests under capacities")
    ...
    def _deterministic_cost(
        ...
        for vid, route in plan.routes.items():
            ...
            for req in route.stops:
                ...
                t += travel
                cur = req.node
            if return_to_depot and route.stops:
